...
io.on("connection", (socket) => {
    socket.on("new player", (name) => {
        ...
        let leftWallShape = new p2.Box({
            ...
            collisionGroup: COLLISION_GROUPS.OBJECT,
            collisionMask: COLLISION_GROUPS.ALL
        });
        leftWallBody.addShape(leftWallShape);
        games[roomId].physics.addBody(leftWallBody);
        ...
        games[roomId].physics.addBody(rightWallBody);
        ...
        games[roomId].physics.addBody(topWallBody);
        ...
        games[roomId].physics.addBody(bottomWallBody);
        ...
        games[roomId].players[socket.id] = {
            ...
            team: team,
            pos: {
                x: Math.floor(Math.random() * (WAITING_MAP_SIZE - PLAYER_SIZE * 2)) + PLAYER_SIZE,
        ...
        playerBody.addShape(new p2.Circle({
            radius: PLAYER_SIZE,
            collisionGroup: COLLISION_GROUPS.PLAYER,
            collisionMask: COLLISION_GROUPS.OBJECT
        }));
        ...
        switch(games[i].state){
            ...
            case STATES.WAITING:
                ...
                if(games[i].waitTime === 0){
                    ...
                    games[i].physics = new p2.World({});
                    for(let j in games[i].players){
                        //morph into playing state
                        games[i].players[j].pos.x = Math.floor(Math.random() * (MAP_SIZE - PLAYER_SIZE * 2)) + PLAYER_SIZE;
                        games[i].players[j].pos.y = Math.floor(Math.random() * (MAP_SIZE - PLAYER_SIZE * 2)) + PLAYER_SIZE;
                        ...
                        let baseBody = new p2.Body({
                            mass: 0,
                            position: [MAP_SIZE / 2, BASE_SIZE / 2 + PLAYER_SIZE * 2],
                            id: "basered"
                        });
                        baseBody.addShape(new p2.Box({
                            width: BASE_SIZE,
                            height: BASE_SIZE,
                            collisionGroup: COLLISION_GROUPS.OBJECT,
                            collisionMask: COLLISION_GROUPS.PLAYER
                        }));
                        games[i].physics.addBody(baseBody);
